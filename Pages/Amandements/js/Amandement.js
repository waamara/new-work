document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateTable(amendments) {
        ...
        amendments.forEach((amendment) => {
            ...
            row.innerHTML = `
                ...
                <td>${amendment.observation || 'N/A'}</td>
                <td>${amendment.type_label || 'Inconnu'}</td> <!-- Display the human-readable label -->
                <td><a href="${amendment.document_path}" target="_blank">Voir le document</a></td>
                <td>   
                    <div>
    ...
    form.addEventListener("submit", (e) => {
        ...
        if (!observation || !observation.value) {
            const message = document.querySelector("#Observation + .validation-message");
            if (message) message.style.display = "block";
            isValid = false;
        }
